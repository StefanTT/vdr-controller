...
public class VdrRecording implements Serializable, Named, VirtualFolderEntry
{
    ...
    /**
     * Update the detailed recording from the list entry.
     *
     * @param recording The list entry to update form
     */
    public void updateFromListEntry(VdrRecording recording)
    {
        this.setTitle(recording.getTitle());
        this.setDuration(recording.getDuration());
        initName();
    }

    /**
     * @return True if the recording contains details, false if it is only the short list
     *         representation.
     */
    ...
    {
        int duration = rec.getDuration();
        if (duration == 0)
            return (int) ((rec.getEndTime().getTimeInMillis() - rec.getStartTime().getTimeInMillis()) / 60000);
        return duration;
    }
